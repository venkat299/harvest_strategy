...
const data = mock_dt.data.candles[0].reduce(function (result, field, index) {
  result[columns[index]] = field;
  return result;
}, {});
data.testing = true;
const updated_order_sell_mock = {
  'entity$': {
    'name': 'order_log',
  },
...
const default_api_test = function (err, val, cb) {
...
function intialize(done) {
  intialize_server.get_server(function (options) {
    ...
    const entity_3 = seneca.make$('watchlist', {
      strategy_id: 'fifty_2_wk',
      tradingsymbol: 'YESBANK',
      status: 'ACTIVE',
      update_time: null,
