...
function reset_expired(opt, callback) {
  ...
  const signal_log_remove$ = Promise.promisify(signal_log.remove$, {
    context: signal_log,
  });
  const signal_log_save$ = Promise.promisify(signal_log.remove$, {
    context: signal_log,
  });
  signal_log_list$().then((val) => {
    ...
    val.forEach((item) => {
      ...
      if (item.signal_status === 'PENDING_OPEN') {
        promise_arr.push(signal_log_remove$({
          id: item.id,
        }));
      }
      if (item.signal_status === 'PENDING_CLOSE') {
        item.signal_status = 'OPEN';
        promise_arr.push(signal_log_save$(item));
      }
    });
    Promise.all(promise_arr).then((e) => {
      logger.debug(' all signals resetted');
      callback(null, {
        success: true,
      });
    });
...
function delete_by_strategy(opt, callback) {
  const seneca = this;
  const signal_log = seneca.make$('signal_log');
  const signal_log_list$ = Promise.promisify(signal_log.list$, {
    context: signal_log,
...
function signal_log_api() {
  ...
  /* Deletes the signal for a strategy, irrespective of its current state;
   * Caution should be taken as it will completely remove a history of signal
   */
  this.add('role:signal_log,cmd:delete_by_strategy', delete_by_strategy.bind(seneca));
  /* ====== INFO ======
