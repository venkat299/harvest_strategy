    ...
    val.forEach((item) => {
      ...
      if (item.signal_status === 'PENDING_OPEN') {
        item.remove$({
          ...
          console.log('deleted signal_log', result);
        });
      }
      if (item.signal_status === 'PENDING_CLOSE') {
        ...
        item.save$((err, result) => {
          if (err) console.log(err);
          console.log('changed signal_log status', result);
        });
      }
    });
    callback(null, {
...
const revert_signal = function (opt, callback) {
...
function check_pending_order(opt, callback) {
  ...
  signal_log_list$({
    ...
    const filtered_signals = val.filter((elem) => (elem.signal_status === 'PENDING_OPEN'
        || elem.signal_status === 'OPEN'
        || elem.signal_status === 'PENDING_CLOSE'));
